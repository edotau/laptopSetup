...
# Set directories to install gonomics
vertgenlab=${GITHUB}/vertgenlab
gonomics=${vertgenlab}/gonomics

# If gonomics repo already exists, we do not perform a git clone
if ! [ -e "$gonomics" ] ; then
    git clone https://github.com/vertgenlab/gonomics.git $gonomics
fi

cd $gonomics
# Contingent on passing all go tests, we finally install/build all gonomics binary programs
go test ./...
go install ./...
